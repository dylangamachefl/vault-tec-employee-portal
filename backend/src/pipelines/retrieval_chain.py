...
Phase 3: Embeddings are now generated in the browser (Transformers.js) and
passed in via the `vector` field on QueryInput. SentenceTransformer has been
removed from this module. A vector MUST be provided for queries initiated from
the API layer.
"""
...
class QueryInput(BaseModel):
    ...
    # Phase 3: pre-computed embedding from the browser (Transformers.js).
    # Required for API-initiated queries; None is only accepted in direct
    # unit-test calls that mock hybrid_retrieve.
...
class VaultRetriever:
    ...
    def __init__(self, collection_name: str = "vault_documents_256") -> None:
        ...
        # Phase 3: embeddings are generated by the browser (Transformers.js).
        # SentenceTransformer has been removed — no model is loaded here.
    ...
    def retrieve(
        ...
        """
        Run hybrid search (dense + sparse RRF) against Qdrant and return raw results.

        Phase 3: `embedding` is the pre-computed vector from the browser.
        If None, raises a ValueError — the backend no longer carries a local
        embedding model.

        access_level_filter: Legacy string filter (kept for backward compat).
        access_filter:       Phase 2 RBAC scaffold — multi-value dict.
        """
        if embedding is None:
            raise ValueError(
                "No embedding provided. Phase 3 requires a pre-computed vector "
                "from the browser (Transformers.js). Pass vector via QueryInput."
            )
    ...
    def query(self, input: QueryInput, access_filter: dict | None = None) -> CitedResponse:
        ...
        Args:
            input:         QueryInput with query, top_k, legacy access_level_filter,
                           and (Phase 3) the pre-computed browser vector.
